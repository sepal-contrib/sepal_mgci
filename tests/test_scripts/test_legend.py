...
def _values(widget):
    return [o["value"] for o in widget.selector_options]


def test_register_appends_and_auto_selects():
    w = MapLegendWidget()
    w.register("land_cover_2020", "Land cover 2020", LegendData())
    assert _values(w) == ["land_cover_2020"]
    assert w.selected == "land_cover_2020"


def test_register_same_key_dedupes_and_keeps_order():
    w = MapLegendWidget()
    w.register("a", "A", LegendData())
    w.register("b", "B", LegendData())
    w.register("a", "A2", LegendData())
    assert _values(w) == ["a", "b"]
    assert w.selector_options[0]["text"] == "A2"
    assert w.selected == "a"
...
def test_clear_empties_everything():
    ...
    w.register("a", "A", LegendData())
    w.clear()
    assert w.selector_options == []
    assert w.selected is None
    assert w.legend_data == {}


def test_clear_thematic_keeps_biobelt():
    w = MapLegendWidget()
    w.register(BIOBELT_KEY, "Bioclimatic belts", LegendData())
    w.register("land_cover_2020", "Land cover 2020", LegendData())
    w.clear_thematic()
    assert _values(w) == [BIOBELT_KEY]
    assert w.selected == BIOBELT_KEY


def test_vue_set_selected_switches_layer():
    w = MapLegendWidget()
    w.register("a", "A", LegendData())
    w.register("b", "B", LegendData())
    assert w.selected == "b"
    w.vue_set_selected("a")
    assert w.selected == "a"
