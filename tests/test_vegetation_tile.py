...
def test_default_matrix(mgci_model):
    """Test default values from mapping land cover matrix"""

    vegetation_tile = VegetationTile(mgci_model)
    w_questionnaire = vegetation_tile.vegetation_view.w_questionnaire
    ...
    # A custom land cover map gets an empty matrix: the default ESA-CCI -> MGCI
    # matrix only makes sense for the default land cover, not a user's own map.
    w_questionnaire.ans_custom_lulc = True
    w_questionnaire.ans_reclassify_custom_lulc = True

    assert mgci_model.matrix_sub_a == {}
    assert mgci_model.matrix_sub_b == {}

    # Not reclassifying the custom map keeps the matrices empty (no remap applies).
    w_questionnaire.ans_reclassify_custom_lulc = False

    assert mgci_model.matrix_sub_a == {}
    assert mgci_model.matrix_sub_b == {}

    # Going back to the default land cover restores the default matrices.
    w_questionnaire.ans_custom_lulc = False
...
def test_target_classes(mgci_model, default_target_classes):
    ...
    # Selecting a custom classification file through the target dialog must
    # propagate to lulc_classes_sub_b on the model.
    ...
    # The file must live under the dialog FileInput's root (the results dir).
    ...
    target_dialog = (
        vegetation_tile.vegetation_view.reclassify_tile_b.w_reclass.target_dialog
    )

    # Simulate the user selecting the file. The frontend sets the ``value`` trait
    # directly; FileInput.select_file is not used (it mishandles Path objects).
    target_dialog.w_dst_class_file.value = str(path_to_file)

    # Validate + load the target classes into the model.
    target_dialog.action_btn.fire_event("click", {})
