...
class LayerHandler(sw.Layout):
    ...
    def update_layer_list(self, change):
        ...
        # Drop the thematic layers and their legends, keeping the biobelt layer
        # (by display name) and its legend (by BIOBELT_KEY) — keep the two in sync.
        self.map_.remove_all(keep_names=["AOI", cm.aoi.legend.belts])
        self.map_.legend.clear_thematic()
    ...
    async def add_layer_async(self, *_):
        ...
        self.map_.legend.register(selection[2], selection[2], legend_data)
